...
#'
#'@details For a given "finite_bin_limits" it creates bins considering also the arguments "include_negInf" and "include_Inf".
...
#'@param column_to_bin Column whose values are used for binning.
#'@param fraction_column Column used to identify the fraction number. Default value is "Fraction_Number".
#'@param finite_bin_limits Finite delimiters between bins.
#'@param include_negInf Include negative infinity as the lower boundary of the first bin. Defaults to FALSE.
#'@param include_Inf Include infinity as the upper boundary of the last bin. Defaults to FALSE.
#'
#'@return A data frame with fractions as rows and bins as columns. Cells report a number of elements in a fraction
#'with values in a bin's range. Additional columns are "NA_values", "total",
#'"min" and "max". The last row reports total number of elements per bin.
#'
#'@export
fracTable <- function(data, fraction_column="Fraction_Number", column_to_bin, finite_bin_limits, include_negInf=FALSE, include_Inf=FALSE){
  ...
  if(any(!inherits(c(include_negInf, include_Inf), "logical"))){
  ...
  number_of_bins <- length(finite_bin_limits) + include_negInf + include_Inf -1
  bins_lowerBoundaries <- c(-Inf, finite_bin_limits)[c(include_negInf, rep(TRUE, length(finite_bin_limits)-1), include_Inf)]
  bins_upperBoundaries <- c(finite_bin_limits, Inf)[c(include_negInf, rep(TRUE, length(finite_bin_limits)-1), include_Inf)]
  ...
  if(na_values_present){
    warning('NA values are present in "column_to_bin". They will be excluded from count and reported in the output column "NA_values".')
  }
